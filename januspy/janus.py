...
import subprocess
import time
from config import (JANUS_SCHEMA, JANUS_HOST, JANUS_PORT)

__version__ = '0.1.0'

janus_url = JANUS_SCHEMA + '://' + JANUS_HOST + ':' + str(JANUS_PORT) + '/janus'
...
def stream(vmin=videorate_min, vmax=videorate_max, amin=audiorate_min, amax=audiorate_max, session=None):
    ...
    if session["streamer"]:
        ...
        for i in range(0, nstreams):
            ...
            args += "      rtpopuspay ! udpsink host=" + str(session["hosts"][i * 2]) + " port=" + str(
                session["ports"][i * 2]) + "  "
            ...
            args += "      rtpvp8pay ! udpsink host=" + str(session["hosts"][i * 2]) + " port=" + str(
                session["ports"][i * 2 + 1]) + " "
        # args += ">/dev/null 2>&1"
        print("Running: " + args)
